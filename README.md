...
- [x] 実機（EAS開発ビルド）での動作確認 — Sign in with Apple→録音→アップロード→処理中→記事プレビュー・編集→履歴→プッシュ通知→アカウント削除まで一通り確認済み。App Store審査へ提出済み（結果待ち）
...
- **X自動投稿（Phase2、優先度: 高）**: バックエンドの`POST /v1/articles/:id/post-to-x`実装に合わせて、設定画面に「自動投稿」ON/OFFのトグルを追加する
- **Note連携の再検討**: 現状は非公式APIのリスクを避け自動投稿を採用していない（§9-1）。Note側で公式APIが提供された場合は改めて対応を検討する
- **位置情報連携**: 散歩ルート・地名を記事に自動挿入する機能（要調査）
- **録音まわりの残タスク**: 30分上限での自動停止、録音の破棄（キャンセル）機能、音量メーターの実音声連動化（現状は録音中/停止中の切り替えのみ）
- **UI/UXの磨き込み**: アニメーション・余白・フィードバックの調整（現状はワイヤーフレーム準拠の実装まで）
